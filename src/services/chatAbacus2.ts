...
import { historyMessageLength, currentModel, useConfig } from './appConfigAbacus.ts'
import { useAI } from './useAbacus.ts'
import { Conversation, Message, useApi } from './apiAbacus2.ts'
...
export function useChats() {
  ...
  const { abort, getAllChats, getChat, updateNameChat } = useApi()
  ...
  const initialize = async (deploymentId: string) => {
    try {
      chats.value = await getAllChats(deploymentId, '30')
  ...
  const switchChat = async (deploymentId: string, deploymentConversationId: string) => {
    try {
      const chat = await getChat(deploymentId, deploymentConversationId)
      if (chat) {
        setActiveChat(chat)
  ...
  const renameChat = async (deploymentId: string, deploymentConversationId: string, newName: string) => {
    if (!activeChat.value) return
    ...
    await updateNameChat(deploymentId, deploymentConversationId, newName)
    chats.value = await getAllChats(deploymentId, '30')
  }
